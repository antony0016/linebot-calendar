...
def current_weather(address):
    ...
    # 定義產生回傳訊息的函式
    def msg_content(loc, raw_msg):
        a = raw_msg
        for i in loc:
            if i in address:  # 如果地址裡存在 key 的名稱
                temperature = f"氣溫 {loc[i]['temperature']} 度，"
                humidity = f"相對濕度 {loc[i]['humidity']}%，"
                r24 = f"累積雨量 {loc[i]['rain_24hr']}mm"
        ...
        for i in city_list:
            if i not in area_list2:  # 將主要縣市裡的數值平均後，以主要縣市名稱為 key，再度儲存一次，如果找不到鄉鎮區域，就使用平均數值
                area_list2[i] = {'temp': round(statistics.mean(city_list[i]['temp']), 1),
                                 ...
                                 'rain_24hr': round(statistics.mean(city_list[i]['rain_24hr']), 1)
                                 }
